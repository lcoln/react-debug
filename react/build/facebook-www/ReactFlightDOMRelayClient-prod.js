  ...
  if (typeof value === 'object' && value !== null) {
    if (isArray(value)) {
      ...
      for (const innerKey in value) {
        parsedValue[innerKey] = parseModelRecursively(response, value, value[innerKey]);
      }
...
function initializeModuleChunk(chunk) {
  const value = ReactFlightDOMRelayClientIntegration.requireModule(chunk._value);
  const initializedChunk = chunk;
  initializedChunk._status = INITIALIZED;
  initializedChunk._value = value;
  return value;
...
function resolveModule(response, id, model) {
  ...
  const moduleReference = resolveModuleReference(response._bundlerConfig, moduleMetaData); // TODO: Add an option to encode modules that are lazy loaded.
...
function resolveSymbol(response, id, name) {
  const chunks = response._chunks; // We assume that we'll always emit the symbol before anything references it
  // to save a few bytes.
