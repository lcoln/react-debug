...
const enableProfilerNestedUpdatePhase = true;
const enableProfilerNestedUpdateScheduledHook =  dynamicFeatureFlags.enableProfilerNestedUpdateScheduledHook;

const enableSchedulingProfiler =  dynamicFeatureFlags.enableSchedulingProfiler; // Note: we'll want to remove this when we to userland implementation.
const enableSuspenseCallback = true;
// don't have to add another test dimension. The build system will compile this
// to the correct value.

const enableNewReconciler = true;
...
function pushRootTransition(workInProgress, root, renderLanes) {
  if (enableTransitionTracing) {
    ...
    push(transitionStack, rootTransitions);
  }
}
...
function pushMarkerInstance(workInProgress, markerInstance) {
  if (enableTransitionTracing) {
    if (markerInstanceStack.current === null) {
      ...
      push(markerInstanceStack, markerInstanceStack.current.concat(markerInstance));
    }
  }
}
...
function commitTransitionProgress(offscreenFiber) {
  if (enableTransitionTracing) {
    ...
    const pendingMarkers = offscreenInstance.pendingMarkers; // If there is a name on the suspense boundary, store that in
        ...
        if (fiber.mode & ProfileMode) {
          ...
          while (current !== null) {
            if (current.tag === Profiler) {
              const _current$memoizedProp = current.memoizedProps,
...
function flushPassiveEffectsImpl() {
  ...
  if (enableTransitionTracing) {
    ...
    if (prevPendingTransitionCallbacks !== null && prevRootTransitionCallbacks !== null) {
      ...
      scheduleCallback$2(IdlePriority, () => processTransitionCallbacks(prevPendingTransitionCallbacks, endTime, prevRootTransitionCallbacks));
    }
  }
