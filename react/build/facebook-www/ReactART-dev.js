...
var disableInputAttributeSyncing = dynamicFeatureFlags.disableInputAttributeSyncing,
    ...
    enableClientRenderFallbackOnTextMismatch = dynamicFeatureFlags.enableClientRenderFallbackOnTextMismatch,
...
var REACT_TRACING_MARKER_TYPE = usePolyfill ? 0xeae5 : Symbol.for('react.tracing_marker');
var REACT_SERVER_CONTEXT_DEFAULT_VALUE_NOT_LOADED = usePolyfill ? 0xeae7 : Symbol.for('react.default_value');
var MAYBE_ITERATOR_SYMBOL = usePolyfill ? typeof Symbol === 'function' && Symbol.iterator : Symbol.iterator;
...
function logComponentSuspended(componentName, wakeable) {
  {
    if (enableDebugTracing) {
      ...
      log$1("%c\u269B\uFE0F%c " + componentName + " suspended", REACT_LOGO_STYLE, 'color: #80366d; font-weight: bold;', id, display);
      wakeable.then(function () {
        log$1("%c\u269B\uFE0F%c " + componentName + " resolved", REACT_LOGO_STYLE, 'color: #80366d; font-weight: bold;', id, display);
      }, function () {
        log$1("%c\u269B\uFE0F%c " + componentName + " rejected", REACT_LOGO_STYLE, 'color: #80366d; font-weight: bold;', id, display);
      });
    }
  }
}
...
function logForceUpdateScheduled(componentName, lane) {
  {
    if (enableDebugTracing) {
      log$1("%c\u269B\uFE0F%c " + componentName + " forced update %c(" + formatLanes(lane) + ")", REACT_LOGO_STYLE, 'color: #db2e1f; font-weight: bold;', '');
    }
  }
}
function logStateUpdateScheduled(componentName, lane, payloadOrAction) {
  {
    if (enableDebugTracing) {
      log$1("%c\u269B\uFE0F%c " + componentName + " updated state %c(" + formatLanes(lane) + ")", REACT_LOGO_STYLE, 'color: #01a252; font-weight: bold;', '', payloadOrAction);
    }
  }
}
...
function mountClassInstance(workInProgress, ctor, newProps, renderLanes) {
  ...
  if (typeof instance.componentDidMount === 'function') {
    ...
    if ( enableStrictEffects && (workInProgress.mode & StrictEffectsMode) !== NoMode) {
...
function pushPrimaryTreeSuspenseHandler(handler) {
  ...
  if ( props.unstable_avoidThisFallback === true && handlerOnStack !== null && !shouldAvoidedBoundaryCapture(handler, handlerOnStack)) {
        ...
        get current() {
          if (!hasBeenInitialized) {
            ...
            if (lazyInitGetterStack === null || lazyInitGetterStack !== getCallerStackFrame()) {
              ...
              warn('%s: Unsafe read of a mutable value during render.\n\n' + 'Reading from a ref during render is only safe if:\n' + '1. The ref value has not been updated, or\n' + '2. The ref holds a lazily-initialized value that is only set once.\n', getComponentNameFromFiber(currentlyRenderingFiber$1) || 'Unknown');
            }
          }
...
function mountEffect(create, deps) {
  if ( enableStrictEffects && (currentlyRenderingFiber$1.mode & StrictEffectsMode) !== NoMode) {
    ...
    case TracingMarkerComponent:
      {
        if (enableTransitionTracing) {
          ...
          if (current === null || (workInProgress.subtreeFlags & Visibility) !== NoFlags) {
            // If any of our suspense children toggle visibility, this means that
            // the pending boundaries array needs to be updated, which we only
            // do in the passive phase.
            workInProgress.flags |= Passive;
...
function commitTransitionProgress(offscreenFiber) {
  if (enableTransitionTracing) {
    ...
    var pendingMarkers = offscreenInstance.pendingMarkers; // If there is a name on the suspense boundary, store that in
          ...
          if (pendingBoundaries !== null && !pendingBoundaries.has(offscreenInstance)) {
            ...
            if (transitions !== null) {
              if (markerInstance.name) {
                addMarkerProgressCallbackToPendingTransition(markerInstance.name, transitions, pendingBoundaries);
              } else {
                transitions.forEach(function (transition) {
...
function getRetryCache(finishedWork) {
  ...
  switch (finishedWork.tag) {
    ...
    case OffscreenComponent:
      {
        var instance = finishedWork.stateNode;
        var _retryCache = instance.retryCache;

        if (_retryCache === null) {
          _retryCache = instance.retryCache = new PossiblyWeakSet();
        }
...
function commitTracingMarkerPassiveMountEffect(finishedWork) {
  ...
  if (instance.transitions !== null && (instance.pendingBoundaries === null || instance.pendingBoundaries.size === 0)) {
    instance.transitions.forEach(function (transition) {
...
var workInProgressRootRenderLanes = NoLanes; // A contextual version of workInProgressRootRenderLanes. It is a superset of
          ...
          while (current !== null) {
            if (current.tag === Profiler) {
              var _current$memoizedProp = current.memoizedProps,
                  ...
                  onNestedUpdateScheduled = _current$memoizedProp.onNestedUpdateScheduled;
  ...
  if (enableTransitionTracing) {
    ...
    if (prevPendingTransitionCallbacks !== null && prevRootTransitionCallbacks !== null) {
      ...
      scheduleCallback$2(IdlePriority, function () {
        return processTransitionCallbacks(prevPendingTransitionCallbacks, endTime, prevRootTransitionCallbacks);
      });
    }
  }
...
exports.Text = Text;
//# sourceMappingURL=ReactART-dev.js.map
